...
module.exports = {
  ...
  execute (message, args) {
    ...
    if (draw.special) {
      message.channel.send(`🎉 Special! 🎉\n${message.author.id} has drawn ${draw.difficulty}: ${draw.name}: ${draw.description}.`)
    } else {
      message.channel.send(`${message.author.id} has drawn ${draw.difficulty}: ${draw.name} words.`)
    }
  }
}
